...
//               <Typography className='link-breadcrumb' variant="h3" sx={{fontSize:"20px"}}>{eachBreadCrumb.title}</Typography>
//             </Link>
      ...
      <div style={{ display: 'flex' }}>
        {breadCrumbsArr.slice(1).map((eachBreadCrumb, index) => (
          <div style={{ display: 'flex' }} key={index + 1}>
            {eachBreadCrumb.link ? (
              <Link to={eachBreadCrumb.link} className='link-to' style={{ display: 'flex' }}>
                <Typography className='link-breadcrumb' variant="h3" sx={{ fontSize: '13px' }}>
            ...
            {index < breadCrumbsArr.length - 2 && <span className='breadcrumb-slash'>/</span>}
          </div>
        ))}
      </div>
    </div>
